...
class _UploadMarksState extends State<UploadMarks> {
  ...
  late String _confirmedSubject, _confirmedCourse, _confirmedDivision;
  late String _confirmedExamType, _confirmedTotalMarks, _confirmedDate;
  ...
  @override
  void dispose() {
    _subjectCtrl.dispose(); _courseCtrl.dispose(); _divisionCtrl.dispose();
    _examTypeCtrl.dispose(); _totalMarksCtrl.dispose(); _dateCtrl.dispose();
  ...
  Future<void> saveMarks() async {
    if (marksControllers.values.any((c) => c.text.trim().isEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Please enter marks for all students"), backgroundColor: Colors.red));
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Marks Uploaded Successfully!"), backgroundColor: Colors.green));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(_formSubmitted ? "Enter Student Marks" : "Upload Marks",
            style: GoogleFonts.playfairDisplay(color: Colors.white, fontSize: 20)),
        centerTitle: true,
        leading: IconButton(
          icon: const Icon(Icons.arrow_back, color: Colors.white),
          onPressed: () { if (_formSubmitted) setState(() => _formSubmitted = false); else Navigator.pop(context); },
        ),
      ),
      body: Column(children: [
        Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Container(height: 2, color: _formSubmitted ? Colors.white : Colors.white38)),
            _stepDot(2, "Enter Marks", _formSubmitted),
          ]),
        ),
        Expanded(child: _formSubmitted ? _buildMarksList() : _buildForm()),
      ]),
    );
  }
    ...
    return SingleChildScrollView(
      ...
      child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFF1E3A5F).withValues(alpha: 0.07),
              borderRadius: BorderRadius.circular(14)),
          child: Row(children: [
            const Icon(Icons.info_outline, color: Color(0xFF1E3A5F), size: 20),
            const SizedBox(width: 10),
            Expanded(child: Text("Fill exam details first, then enter marks for each student.",
                style: TextStyle(fontSize: 13, color: Colors.grey.shade700))),
          ]),
        ),
        const SizedBox(height: 24),
        _sLabel("Subject Details"),
        const SizedBox(height: 12),
        _inp(controller: _subjectCtrl, label: "Subject Name", icon: Icons.menu_book,
            validator: (v) => v!.isEmpty ? "Enter subject name" : null),
        const SizedBox(height: 14),
        Row(children: [
          Expanded(child: _inp(controller: _courseCtrl, label: "Course", icon: Icons.school,
              validator: (v) => v!.isEmpty ? "Required" : null)),
          const SizedBox(width: 12),
          Expanded(child: _inp(controller: _divisionCtrl, label: "Division", icon: Icons.group,
              validator: (v) => v!.isEmpty ? "Required" : null)),
        ]),
        const SizedBox(height: 24),
        _sLabel("Exam Details"),
        const SizedBox(height: 12),
        _inp(controller: _examTypeCtrl, label: "Exam Type (e.g. Mid-Term, Final)", icon: Icons.assignment,
            validator: (v) => v!.isEmpty ? "Enter exam type" : null),
        const SizedBox(height: 14),
        Row(children: [
          Expanded(child: _inp(controller: _totalMarksCtrl, label: "Total Marks", icon: Icons.score,
              keyboardType: TextInputType.number, validator: (v) => v!.isEmpty ? "Required" : null)),
          const SizedBox(width: 12),
          Expanded(child: _inp(controller: _dateCtrl, label: "Date", icon: Icons.calendar_today,
              validator: (v) => v!.isEmpty ? "Required" : null)),
        ]),
        ...
        SizedBox(width: double.infinity, height: 52,
          child: ElevatedButton.icon(
            ...
            label: const Text("Proceed to Enter Marks",
    ...
    return Column(children: [
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(_confirmedSubject, style: const TextStyle(fontWeight: FontWeight.bold,
              fontSize: 15, color: Color(0xFF1E3A5F))),
          const SizedBox(height: 6),
          Wrap(spacing: 8, runSpacing: 6, children: [
            _chip("${_confirmedCourse}-${_confirmedDivision}", Icons.school),
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                final d          = students[index].data() as Map<String, dynamic>;
                  ...
                  child: IntrinsicHeight(child: Row(children: [
                    ...
                    Expanded(child: Padding(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text(rollNumber, style: const TextStyle(fontSize: 12,
                            color: Color(0xFF1E3A5F), fontWeight: FontWeight.w600)),
                      ]),
                    )),
                    SizedBox(width: 80,
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          filled: true,
                          fillColor: const Color(0xFF1E3A5F).withValues(alpha: 0.05),
  ...
  Widget _stepDot(int num, String label, bool active) => Column(mainAxisSize: MainAxisSize.min, children: [
    Container(width: 30, height: 30,
        decoration: BoxDecoration(shape: BoxShape.circle, color: active ? Colors.white : Colors.white38),
        child: Center(child: Text("$num", style: TextStyle(fontWeight: FontWeight.bold,
            color: active ? const Color(0xFF1E3A5F) : Colors.white60)))),
  ...
  Widget _chip(String label, IconData icon) => Container(
    ...
    decoration: BoxDecoration(color: const Color(0xFF1E3A5F).withValues(alpha: 0.1),
  ...
  Widget _inp({required TextEditingController controller, required String label,
