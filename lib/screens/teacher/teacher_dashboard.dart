...
class _TeacherDashboardState extends State<TeacherDashboard> {
  ...
  // Aaj ke taken lectures ka set
  Set<String> _todayTakenLectures = {};
  ...
  // ✅ BULLETPROOF ATTENDANCE CHECKER
    ...
    try {
      ...
      for (var doc in snap.docs) {
        ...
        if (d["date"] is Timestamp) {
          ...
          if (d["date"] == todayStr1 || d["date"] == todayStr2) {
            isToday = true;
          }
        }

        if (isToday) {
          final subject = (d["subject"] ?? d["subjectName"] ?? "").toString().trim();
          final lNo = (d["lectureNo"] ?? 1).toString();
          taken.add("${subject}_$lNo");
        }
      }

      if (mounted) setState(() => _todayTakenLectures = taken);
    } catch (e) {
  ...
  Future<void> _loadPendingAttendance() async {
    try {
      ...
      for (int i = 1; i <= 7; i++) {
        ...
        final targetDateStr1 = "${date.day.toString().padLeft(2,'0')}-${date.month.toString().padLeft(2,'0')}-${date.year}";

        for (var lecture in dayLectures) {
          final ld          = lecture.data();
          final subjectName = (ld["subjectName"] as String? ?? "").trim();
          final lectureType = ld["lectureType"] as String? ?? "LEC";
          final semester    = ld["semester"]    as String? ?? "";
          final lectureNo   = (ld["lectureNo"]  as num?)?.toInt() ?? 1;
          ...
          for (var doc in attSnap.docs) {
            ...
            if (d["date"] is Timestamp) {
              ...
              if (d["date"] == targetDateStr1) isMatchingDate = true;
            }

            if (isMatchingDate) {
              final dbSubj = (d["subject"] ?? d["subjectName"] ?? "").toString().trim();
              final dbLNo  = (d["lectureNo"] ?? 1).toString();
              if (dbSubj == subjectName && dbLNo == lectureNo.toString()) {
                found = true;
                break;
              }
            }
  ...
  Future<void> _showSubjectPicker(BuildContext context) async {
    ...
    try {
      ...
      for (var doc in snap.docs) {
        ...
        final semester = d["semester"]    as String? ?? "";
        if (subject.isEmpty) continue;
        unique.putIfAbsent("$subject||$course||$division||$semester", () =>
      ...
      showModalBottomSheet(
        ...
        builder: (_) => DraggableScrollableSheet(
          ...
          maxChildSize: 0.85, minChildSize: 0.3,
              ...
              Expanded(child: ListView.separated(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(
                        "${s["course"]} - ${s["division"]} (${s["semester"]})",
                        style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        child: Container(
          ...
          child: SafeArea(child: Column(children: [
            Expanded(child: SingleChildScrollView(child: Column(children: [
              ...
              _dItem(context, Icons.dashboard, "Dashboard",
                      () => Navigator.pop(context)),
              ...
              _dItem(context, Icons.history, "Previous Attendance", () {
                ...
                Navigator.push(context, MaterialPageRoute(
                    builder: (_) =>
                        PreviousAttendance(teacherId: widget.teacherId)));
              }),
            ]))),
      ...
      appBar: AppBar(
        ...
        title: Text("Welcome ${widget.teacherName}", // Wapas proper title
            style: GoogleFonts.playfairDisplay(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, ttSnap) {
          final lectures = ttSnap.hasData
            ...
            ..sort((a, b) {
              final an = ((a.data() as Map)["lectureNo"] as num?)
                  ?.toInt() ?? 0;
              final bn = ((b.data() as Map)["lectureNo"] as num?)
                  ?.toInt() ?? 0;
          ...
          return SingleChildScrollView(child: Column(children: [
            ...
            Padding(
              ...
              child: GridView.count(
                ...
                mainAxisSpacing: 16,
                children: [
                  _aCard(context, Icons.upload, "Upload Marks",
                          () => _showSubjectPicker(context)),
                  _aCard(context, Icons.bar_chart, "View Marks",
                          () => Navigator.push(context, MaterialPageRoute(
                          builder: (_) => ViewMarks(
                              teacherId: widget.teacherId,
                              collegeId: widget.collegeId)))),
                  _aCard(context, Icons.people, "View Students",
                          () => Navigator.push(context, MaterialPageRoute(
                          builder: (_) =>
                              ViewStudents(collegeId: widget.collegeId)))),
                  _aCard(context, Icons.history, "Previous Attendance",
                          () => Navigator.push(context, MaterialPageRoute(
                          builder: (_) => PreviousAttendance(
                              teacherId: widget.teacherId)))),
                  _aCard(context, Icons.calendar_month, "My Timetable",
                          () => Navigator.push(context, MaterialPageRoute(
                          builder: (_) => ViewTimetable(
                              teacherId:   widget.teacherId,
                              teacherName: widget.teacherName,
                              collegeId:   widget.collegeId)))),
                  _aCard(context, Icons.campaign_rounded, "Notice Board",
                          () => Navigator.push(context, MaterialPageRoute(
                          builder: (_) => NoticeBoard(
                              teacherId:   widget.teacherId,
                              teacherName: widget.teacherName,
                      ...
                      child: Row(children: [
                        Container(
                          ...
                          child: Column(children: [
                            ...
                            Text(
                              _monthShort((p["date"] as DateTime).month),
                              style: TextStyle(
                                  fontSize: 11,
            ...
            if (ttSnap.connectionState == ConnectionState.waiting)
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final subjectName = (d["subjectName"] as String? ?? "Unknown").trim();
                  final course      = d["course"]      as String? ?? "";
                  final division    = d["division"]    as String? ?? "";
                  final room        = d["room"]        as String? ?? "TBA";
                  final startTime   = d["startTime"]   as String? ?? "";
                  final endTime     = d["endTime"]     as String? ?? "";
                  final lectureNo   = (d["lectureNo"] ?? 1).toString();
                  final lectureType = d["lectureType"] as String? ?? "LEC";
                  final semester    = d["semester"]    as String? ?? "";

                  final trackingKey = "${subjectName}_$lectureNo";
                  final isTaken = _todayTakenLectures.contains(trackingKey);
                                    ...
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (_) => AttendanceScreen(
                                          ...
                                          teacherId:   widget.teacherId,
                                          collegeId:   widget.collegeId,
                                          lectureNo:   int.tryParse(lectureNo) ?? 1,
                                        ),
                                      ),
                                    ).then((_) {
                                      _loadAllData();
  ...
  Widget _dItem(BuildContext ctx, IconData icon, String title,
      VoidCallback onTap) =>
      ListTile(
  ...
  Widget _aCard(BuildContext ctx, IconData icon, String title,
      VoidCallback onTap) =>
      GestureDetector(
        onTap: onTap,
        child: Container(
